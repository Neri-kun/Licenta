...
class SongReccomender:
    ...
    """ This is the class of a content-based recommender system, which generates track recommendations based on specific Audio features (accousticness, danceability, etc.),
    popularity ratings and lyrics similarity.
        ...
        non_categorical_audio_features: The list of non categorical auido features used for the recommendation system.
        The list contains the following string values representing useful audio track features:
            - 'danceability' : a measure on a scale from 0 to 1 on how suitable a track is for dancing
            - 'energy' : a measure on a scale from 0 to 1 of track intensity
            - 'loudness' : overall loudness value in dB
            - speechiness:
            - 'acousticness' : a measure on a scale from 0 to 1 on how accoustic a track is.
            - 'instrumentalness' : measures on a scale from 0 to 1 the lack of vocal content.
            - 'liveness' : measures on a scale from 0 to 1 the presence of an audicence. Higher values indicate that the track was recorded live.
            - 'valence' : measures on a scale from 0 to 1 how positive a track is
            - 'tempo' : track pace value in BPM (Beats Per Minute)
            - 'duration_ms' : the duration of a track in milliseconds

        new_track_dataframe: the track dataframe after having its audio features into a number of principal component values
    """
    ...
    def __init__(self, filename, number_of_principal_components=6, number_of_clusters=6):
        ...
        self.__remove_categorical_features_and_min_max_selected_features()
        self.__convert_track_dataframe_audio_features_into_pca_features(self.number_of_principal_components)
        self.__cluster_dataframe_in_numbered_labels_with_k_means()
        self.__readd_categorical_features()
        self.__classify_track_labels_by_knn()
        self.__compute_tf_idf_matrices_by_label_for_all_tracks()


    def __remove_categorical_features_and_min_max_selected_features(self):
    ...
    def __convert_track_dataframe_audio_features_into_pca_features(self, number_of_components=6):
    ...
    def __readd_categorical_features(self):
